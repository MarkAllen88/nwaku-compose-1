...
TST_AMOUNT_WEI=$MEMBERSHIP_DEPOSIT
...
echo "Submitting docker run -v" "$(pwd)/keystore":/keystore/:Z wakuorg/nwaku:latest generateRlnKeystore ${RLN_RELAY_ETH_CLIENT_ADDRESS} ${ETH_TESTNET_KEY} ${RLN_CONTRACT_ADDRESS} ${RLN_RELAY_CHAIN_ID} "/keystore/keystore.json" ${RLN_RELAY_CRED_PASSWORD} "100"

# --rln-relay-eth-contract-address=0xB9cd878C90E49F797B4431fBF4fb333108CB90e6 \
# --rln-relay-eth-chain-id=59141 \
# docker run -v "$(pwd)/keystore":/keystore/:Z wakuorg/nwaku:v0.36.0
docker run -v "$(pwd)/keystore":/keystore/:Z wakuorg/nwaku:latest generateRlnKeystore \
